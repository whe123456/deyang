...
$sql="SELECT js_bm FROM zjzz_js WHERE wxid=?";
...
$count_arr=Database::ReadoneStr("SELECT MIN(zq.id) FROM zjzz_qj zq WHERE zq.js_bm=? $where",$conn,array($user));
$sql="SELECT zq.*,zj.xm FROM zjzz_qj zq,zjzz_js zj WHERE zq.js_bm=? AND zq.js_bm=zj.js_bm $where order by zq.id desc limit 0,3";
$arr=Database::Readall($sql,$conn,array($user));
$more=true;
$minid='';
$list=array();
if(count($arr)>0){
